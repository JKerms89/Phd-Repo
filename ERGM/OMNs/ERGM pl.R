...
RCA <- RCA %>% filter(source_country == 'POLAND')
...
# Calculate coefficient(s) of in-group (same nationality) ties
plogis(coef(random_graph5)[['nodematch.act_nat']])
# [1] 0.8279249

# Calculate coefficient(s) of in-group (same actor type) ties
plogis(coef(random_graph5)[['nodematch.act_type']])
# [1] 0.7356619

# Calculate coefficient(s) of transitivty 
#(e.g. x -> y, y -> z, prob(%) x -> z))

plogis(coef(random_graph5)[['gwesp.fixed.0.35']])
# [1] 0.6557538
